<?php
// src/Controller/UserSupportBackhaulController.php
...
#[Route('/user/support-backhaul')]
class UserSupportBackhaulController extends AbstractController
{
    ...
    #[Route('/', name: 'user_support_backhaul_index', methods: ['GET'])]
    public function index(Request $request): Response
    {
        ...
        $qb = $this->taskRepo->createQueryBuilder('t')
            ...
            ->setParameter('dept', 'support_backhaul')
        ...
        return $this->render('dashboard/user/support_backhaul/index.html.twig', [
            'tasks' => $tasks,
            'total' => $total,
            'pending' => $pending,
    ...
    #[Route('/task/{id}', name: 'user_support_backhaul_task_show', methods: ['GET'])]
    public function show(TicketTask $task, Request $request): Response
    {
        ...
        return $this->render('dashboard/user/support_backhaul/show.html.twig', [
            'task' => $task,
            'ticket' => $ticket,
            'sites' => $sites,
    ...
    #[Route('/task/{id}/start', name: 'user_support_backhaul_task_start', methods: ['GET'])]
    public function startTask(TicketTask $task): Response
    {
        ...
        return $this->redirectToRoute('user_support_backhaul_task_show', ['id' => $task->getId()]);
    }

    #[Route('/task/{id}/validate', name: 'user_support_backhaul_validate', methods: ['POST'])]
    public function validate(TicketTask $task, Request $request): Response
    {
        ...
        // Enregistrer la décision dans la tâche (pour historique)
        ...
        foreach ($task->getTicket()->getTicketSites() as $site) {
            $siteDecisions[$site->getId()] = array_merge($siteDecisions[$site->getId()] ?? [], [
                'backhaul_ok' => $ok,
                'backhaul_comment' => $comment,
                'backhaul_validated_at' => (new \DateTime())->format('Y-m-d H:i:s'),
            ]);
        }
        ...
        // ✅ Délégation à la logique centralisée
        $this->deploiementWorkflowService->onSupportTaskValidated($task, 'backhaul', $ok, $comment, $this->getUser());

        $this->ticketWorkflowService->refreshTicketProgress($task->getTicket());

        if ($ok) {
            $this->addFlash('success', 'Validation Support Backhaul enregistrée.');
        } else {
            $this->addFlash('warning', 'Support Backhaul marqué NOK -- le workflow a été bloqué pour ce ticket.');
        }
    ...
    {
        if ($task->getDepartmentName() !== 'support_backhaul') {
            throw $this->createAccessDeniedException('Cette tâche n\'appartient pas au Support Backhaul.');
        }
